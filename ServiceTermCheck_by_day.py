...
def match(df2,df):    
    ...
    #df5.to_excel(file_path + 'result.xlsx',index=False,encoding='gbk')
    return df5


def statis(data,date):
    '''按联通需求的总量统计内容'''
    ...
    statis.to_excel(file_path + './总统计量_{}.xlsx'.format(date),encoding='gbk')
...
file_path = 'C:/Users/xueji/Desktop/data_dealing/'
df,date = prepare_csv(file_path)
r = role_split(file_path,df)
data = match(r,df)
statis(data,date)
save_center_results(data,date)